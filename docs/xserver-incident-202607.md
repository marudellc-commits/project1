...
- [x] royal878.jpを正式復旧・再公開（2026-07-22 17:19〜17:44、本番トラフィック下で25分間・30秒間隔50回監視。常駐プロセス出現ゼロ、`.htaccess`/`index.php`のハッシュ改ざんゼロを確認して復旧）
- [ ] 常駐プロセスの起動トリガー（削除済みバックドア群のいずれかと推定されるが未確定）の事後特定は保留。再発時は`public_html.maintenance_20260722`（メンテナンスページ一式）と`~/incident_evidence_20260722_royal878*.tar.gz`（削除ファイル証跡）が残っているので参照すること
- [ ] royal878.jpのテーマ（noel_tcd072）・プラグイン（really-simple-ssl等）を公式配布元から再取得し、既存ファイルと比較検証（優先度は下げたが未実施）
- [ ] しばらくは高頻度（週1回程度）でroyal878.jpの`ps -ef`・構造検索を再確認する運用に変更
